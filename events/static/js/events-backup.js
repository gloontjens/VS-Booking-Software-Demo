...
//     });
    ...
    dummy = setTimeout(rescan_now, 2500);
});
...
$(document).on('click', '#id_reminderdisable', function() {
    ...
    dummy =  setTimeout(updateReminders, 1000);    
});
...
function evRemindCheck() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#id_event_reminders_done').prop("checked", false);
            updateFlags();
            dummy =  setTimeout(updateActivities, 1500);
            dummy =  setTimeout(updateFlags, 2250);
...
function updateActivities() {
    if (xhr != null) {
        //alert("no updateActivities");
        return;
    }
    //alert("activites update...");
    xhr = "busy";
    var data = {
        "event_id": $('#page_instance').val()
    };
...
//     dummy =  setTimeout(updateMusiciansAskedList, 6000);
    win.close();
    dummy = setTimeout(resaveevent, 400);
}
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data['error'] == true) {
                ...
                dummy = setTimeout(updateMusiciansList, 1000); 
            }
        },
        error: function(data) {
        ...
        success: function(data) {
            if (data['error'] == true) {
                ...
                if (data['fact_sheets_rcvd'] == true) {
                    ...
                    $('#id_event_reminders_done').prop("checked", false);
                    dummy = setTimeout(updateFlags, 150);
                }
                dummy = setTimeout(updateMusiciansAskedList, 1500);
                dummy = setTimeout(updateMusiciansList, 2000); 
            }
        },
        error: function(data) {
...
function updateMusiciansAskedList() {
    if (xhr != null) {
        //alert("no updateMusiciansAskedList");
        return;
    }
    xhr = "busy";
    var data = {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data['error'] == true) {
                ...
                var dummy;
                dummy =  setTimeout(updateMusiciansList, 1500);    
            }
        },
        error: function(data) {
...
$("#id_location_0").on('djselectableselect focusout', function(event, ui) {
    venue_name = $("#id_location_0").val();
    var data = {
        "venue_name": venue_name
    };
...
$("#id_event_type_0").on('djselectablechange', function(event, ui) {
...
$("#id_ensemble_0").on('djselectablechange', function(event, ui) {
...
$("#id_contact_0").on('djselectableselect focusout', function(event, ui) {
    contact_name = $("#id_contact_0").val();
    var data = {
        "contact_name": contact_name
    };
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data['noerror'] == true) {
                ...
                $('#contact_show_phone').html(data['contact_phone']);
                $('#contact_show_email').html(data['contact_email']);
                if ($('#contact_button').prop("checked") == true) {
...
$("#id_dayofcontact_0").on('djselectableselect focusout', function(event, ui) {
    dayofcontact_name = $("#id_dayofcontact_0").val();
    var data = {
        "dayofcontact_name": dayofcontact_name
    };
    $.ajax({
        ...
        success: function(data) {
            if (data['noerror'] == true) {
                ...
                //$('#dayofcontact_button').prop("checked", false);
                if ($('#dayofcontact_button').prop("checked") == true) {
